...
var createServer = (targetVideoPath, port2 = 8080) => __async(void 0, null, function* () {
  let stat;
  try {
    stat = yield import_node_fs.default.promises.stat(targetVideoPath);
  } catch (e) {
    console.error(e);
    return false;
  }
  return new Promise((resolve5) => {
    import_node_http.default.createServer((req, res) => {
      ...
      if (range) {
        ...
        const file = import_node_fs.default.createReadStream(targetVideoPath, { start, end });
        res.writeHead(206, {
        ...
        const file = import_node_fs.default.createReadStream(targetVideoPath);
        res.writeHead(200, {
    ...
    }).listen(port2).once("listening", () => resolve5(true)).once("error", (errMsg) => {
      console.error(errMsg);
      resolve5(false);
    });
  });
});
var create_server_default = createServer;
...
// index.ts
var y = yargs_default();
var startServe = (filePath2, port2) => __async(exports, null, function* () {
  const localIpInternal = (() => {
    const interfaces = import_node_os.default.networkInterfaces();
    for (const iface in interfaces) {
      const ifaceDetail = interfaces[iface].find(
        (ifaceDetails) => ifaceDetails.family === "IPv4" && !ifaceDetails.internal
      );
      if (ifaceDetail) {
  ...
  const createRes = yield create_server_default(filePath2, port2);
  if (createRes) {
    const targetUrl = `http://${localIpInternal}:${port2}/awesome.mp4`;
    const command2 = `open -a /Applications/Safari.app ${targetUrl}`;
...
${command2}`
    ...
    (0, import_node_child_process.exec)(`open -a /Applications/Safari.app ${targetUrl}`);
  }
});
var argv = y.scriptName("awesome-touping").command(
  "serve <path>",
...
}).demandCommand(1, "You need at least one command before moving on").example([
...
var { path: filePath, port } = y.parse(process.argv.slice(2));
